...
\begin{frame}{Hypothesis Class: Linear Classifiers}
  Introduce a dummy feature \(x_0 = 1\) and augmented vector
  \[
    \tilde{\mathbf{x}} = [1, x_1, \ldots, x_n]^T \in \R^{n+1}.
  \]
  Define a single parameter vector
  \[
    \boldsymbol{\theta} = [b, w_1, \ldots, w_n]^T \in \R^{n+1}.
  \]
  Model and prediction:
  \[
    f_{\boldsymbol{\theta}}(\tilde{\mathbf{x}}) = \boldsymbol{\theta}^T \tilde{\mathbf{x}},
    \quad
    \hat{y} = \mathbf{1}[f_{\boldsymbol{\theta}}(\tilde{\mathbf{x}}) \ge 0].
  \]
\end{frame}

\begin{frame}{Geometric Interpretation}
  \begin{itemize}
    \item The hyperplane is \(\boldsymbol{\theta}^T \tilde{\mathbf{x}} = 0\).
    \item Signed distance (up to scale) depends on \(\boldsymbol{\theta}\).
...
\begin{frame}{Linear Separability}
  \begin{itemize}
    \item If classes are linearly separable, some \(\boldsymbol{\theta}\) achieves zero training error.
...
\begin{frame}{Probabilistic Model (Binary)}
  Define a score:
  \[
    s(\tilde{\mathbf{x}}) = \boldsymbol{\theta}^T \tilde{\mathbf{x}}
  \]
  Logistic model:
  \[
    P(y=1 \mid \tilde{\mathbf{x}}) = \sigma(s(\tilde{\mathbf{x}})), \quad \sigma(t) = \frac{1}{1+e^{-t}}
  \]
  \[
    P(y \mid \tilde{\mathbf{x}}) = \text{Bernoulli}(y; \sigma(s(\tilde{\mathbf{x}})))
  \]
\end{frame}

\begin{frame}{Binary NLL (Single Example)}
  For \(y \in \{0,1\}\) and \(p = \sigma(s(\tilde{\mathbf{x}}))\):
  \[
    \ell(\boldsymbol{\theta}; \tilde{\mathbf{x}}, y)
    = -\log P(y \mid \tilde{\mathbf{x}})
...
\begin{frame}{Binary Cross-Entropy (Dataset)}
  ...
  \]
  where \(p^{(i)} = \sigma(\boldsymbol{\theta}^T \tilde{\mathbf{x}}^{(i)})\).
  \begin{itemize}
    \item Convex in \(\boldsymbol{\theta}\).
...
\begin{frame}{Gradient of Binary NLL}
  Let \(\tilde{X} \in \R^{m \times (n+1)}\) stack augmented features and
  \(\mathbf{p} = \sigma(\tilde{X}\boldsymbol{\theta})\).
  \[
    \nabla_{\boldsymbol{\theta}} \mathcal{L} = \frac{1}{m} \tilde{X}^T (\mathbf{p} - \mathbf{y})
  \]
  \begin{itemize}
    \item Gradient is proportional to prediction error \((\mathbf{p} - \mathbf{y})\).
    \item With L2 regularization: add \(\lambda \boldsymbol{\theta}\).
  \end{itemize}
\end{frame}
...
\begin{frame}{Gradient Descent Pseudocode (Binary)}
  \begin{algorithm}[H]
  \SetAlgoLined
  \KwIn{Augmented data $\tilde{X}$, labels $\mathbf{y} \in \{0,1\}^m$, step size $\eta$, steps $T$}
  \KwOut{Parameters $\boldsymbol{\theta}$}
  Initialize $\boldsymbol{\theta} \leftarrow 0$\;
  \For{$t=1$ \textbf{to} $T$}{
    $\mathbf{p} \leftarrow \sigma(\tilde{X}\boldsymbol{\theta})$\;
    $\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \eta \frac{1}{m} \tilde{X}^T (\mathbf{p}-\mathbf{y})$\;
  }
  \end{algorithm}
\end{frame}
...
\begin{frame}{Prediction and Probabilities}
  ...
  \[
    P(y=1 \mid \tilde{\mathbf{x}}) = \sigma(\boldsymbol{\theta}^T \tilde{\mathbf{x}})
  \]
...
\begin{frame}{Feature Engineering}
  \begin{itemize}
    ...
    \item Augment with bias: \(\tilde{\boldsymbol{\phi}}(\mathbf{x}) = [1, \boldsymbol{\phi}(\mathbf{x})]^T\).
  \end{itemize}
  \[
    \hat{y} = \mathbf{1}[\boldsymbol{\theta}^T \tilde{\boldsymbol{\phi}}(\mathbf{x}) \ge 0]
  \]
\end{frame}
...
\begin{frame}{Handling Class Imbalance}
  Common strategies:
  \begin{itemize}
    \item Re-weight the loss: use class weights in NLL.
    \item Resampling: oversample minority or undersample majority.
    \item Adjust threshold using validation data.
  \end{itemize}
  Weighted NLL example:
  \[
    \mathcal{L}(\boldsymbol{\theta}) =
    \frac{1}{m}\sum_{i=1}^m
    \left[-\alpha\, y^{(i)} \log p^{(i)} - \beta\,(1-y^{(i)})\log(1-p^{(i)})\right]
  \]
\end{frame}
