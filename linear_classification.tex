...
\begin{frame}{Hypothesis Class: Linear Classifiers}
  Assume \(\mathbf{x}\) includes a dummy feature \(x_0=1\), so \(\mathbf{x}\in\R^{n+1}\).
  Define a single parameter vector
  \[
    \boldsymbol{\theta} = [\theta_0, \theta_1, \ldots, \theta_n]^T \in \R^{n+1}.
  \]
  Model and prediction:
  \[
    f_{\boldsymbol{\theta}}(\mathbf{x}) = \boldsymbol{\theta}^T \mathbf{x},
    \quad
    \hat{y} = \mathbf{1}[f_{\boldsymbol{\theta}}(\mathbf{x}) \ge 0].
  \]
\end{frame}
...
\begin{frame}{Probabilistic Model (Binary)}
  Define a score:
  \[
    s(\mathbf{x}) = \boldsymbol{\theta}^T \mathbf{x}
  \]
  Logistic model:
  \[
...
\begin{frame}{Binary Cross-Entropy (Dataset)}
  ...
  \]
  where \(p^{(i)} = \sigma(\boldsymbol{\theta}^T \mathbf{x}^{(i)})\).
  \begin{itemize}
...
\begin{frame}{Gradient of Binary NLL}
  Let \(X \in \R^{m \times (n+1)}\) include the dummy feature and
  \(\mathbf{p} = \sigma(X\boldsymbol{\theta})\).
  \[
    \nabla_{\boldsymbol{\theta}} \mathcal{L} = \frac{1}{m} X^T (\mathbf{p} - \mathbf{y})
  \]
  \begin{itemize}
...
\begin{frame}{Gradient Descent Pseudocode (Binary)}
  \begin{algorithm}[H]
  \SetAlgoLined
  \KwIn{Data $X$ (with dummy feature), labels $\mathbf{y} \in \{0,1\}^m$, step size $\eta$, steps $T$}
  \KwOut{Parameters $\boldsymbol{\theta}$}
  Initialize $\boldsymbol{\theta} \leftarrow 0$\;
  \For{$t=1$ \textbf{to} $T$}{
    $\mathbf{p} \leftarrow \sigma(X\boldsymbol{\theta})$\;
    $\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \eta \frac{1}{m} X^T (\mathbf{p}-\mathbf{y})$\;
  }
  \end{algorithm}
\end{frame}
...
\begin{frame}{Softmax Model and Parameters}
  ...
  \begin{itemize}
    ...
    \item Assume \(\mathbf{x}\) already includes the dummy feature.
  \end{itemize}
\end{frame}
...
\begin{frame}{Feature Engineering}
  \begin{itemize}
    ...
    \item Assume \(\boldsymbol{\phi}(\mathbf{x})\) includes the dummy feature.
  \end{itemize}
  \[
    \hat{y} = \mathbf{1}[\boldsymbol{\theta}^T \boldsymbol{\phi}(\mathbf{x}) \ge 0]
  \]
\end{frame}
