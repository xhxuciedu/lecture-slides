...
\begin{frame}{Training vs.\ Test Error}
  ...
  \textbf{Key point:}
  \[
    \mathcal{L}_{\text{train}}(\boldsymbol{\theta}) \neq \mathcal{L}_{\text{test}}(\boldsymbol{\theta})
  \]
\end{frame}
...
\begin{frame}[fragile]{Gradient Descent in Python}
\begin{verbatim}
import numpy as np

def gradient_descent(X, y, eta=0.05, steps=1000):
    m, n = X.shape
    theta = np.zeros(n)
...
\begin{frame}[fragile]{Normal Equations in Python}
\begin{verbatim}
import numpy as np
...
\begin{frame}{Polynomial Regression Example}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[domain=-2:2, samples=200, thick, color=teal] {0.75 + 0.35*x - 0.25*x^2};
      \addlegendentry{Good fit}
      \addplot[domain=-2:2, samples=400, thick, color=magenta]
        {0.2 + 0.9*x - 1.1*x^2 + 0.6*x^3 - 0.15*x^4 + 0.02*x^5};
      \addlegendentry{Overfit (high degree)}
...
\begin{frame}{Training vs.\ Test Error Curves}
  \centering
  \includegraphics[width=0.85\textwidth]{figs/model_complexity_error.png}
\end{frame}
