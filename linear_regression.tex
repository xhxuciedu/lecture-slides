...
\begin{frame}{Synthetic Data Visualization}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      % Residual annotation
      \addplot[dashed, color=black] coordinates {(2,2.9) (2,2.4)};
      \node[anchor=west] at (axis cs:2.05,2.7) {residual};
    \end{axis}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Gradient Descent Pseudocode}
  \begin{algorithm}[H]
  \SetAlgoLined
  \KwIn{Data matrix $X$, targets $y$, step size $\eta$, iterations $T$}
  \KwOut{Parameters $\theta$}
  Initialize $\theta \leftarrow 0$\;
  \For{$t=1$ \textbf{to} $T$}{
    $g \leftarrow \frac{2}{m} X^T (X\theta - y)$\;
...
\begin{frame}{GD Variants: Gradient Estimates}
  ...
  \[
    g_t = \frac{2}{|\mathcal{B}_t|} X_{\mathcal{B}_t}^T (X_{\mathcal{B}_t}\theta - y_{\mathcal{B}_t})
  \]
  \begin{itemize}
