...
\begin{frame}{Training vs.\ Test Error}
  ...
  \begin{block}{Goal}
    ...
    \[
      L(\boldsymbol{\theta}) = \frac{1}{m} \sum_{i=1}^m \ell(h_{\boldsymbol{\theta}}(\mathbf{x}^{(i)}), y^{(i)}).
    \]
  \end{block}
  ...
  \textbf{Key point:}
  \[
    \widehat{R}_{\text{train}}(\boldsymbol{\theta}) \neq \widehat{R}_{\text{test}}(\boldsymbol{\theta})
  \]
\end{frame}
...
\begin{frame}{Dataset in Matrix Form}
  ...
  \[
    \mathbf{X} \in \R^{m \times (n+1)}, \quad \mathbf{y} \in \R^m
  \]
  Predictions:
  \[
...
\begin{frame}{Learning as Optimization}
  ...
  \begin{itemize}
    ...
    \item Unique minimizer if \(\mathbf{X}^T \mathbf{X}\) is full rank.
  \end{itemize}
\end{frame}

\begin{frame}{Geometry of Least Squares}
  \begin{itemize}
    \item Column space of \(\mathbf{X}\) defines all possible predictions.
    \item Optimal \(\hat{\mathbf{y}}\) is orthogonal projection of \(\mathbf{y}\) onto \(\text{col}(\mathbf{X})\).
    \item Residual \(\mathbf{r} = \mathbf{y} - \mathbf{X}\boldsymbol{\theta}^*\) satisfies \(\mathbf{X}^T \mathbf{r} = 0\).
  \end{itemize}
\end{frame}

\begin{frame}{Gradient of the MSE Loss}
  \[
    \nabla_{\boldsymbol{\theta}} \mathcal{L}(\boldsymbol{\theta}) = \frac{2}{m} \mathbf{X}^T (\mathbf{X}\boldsymbol{\theta} - \mathbf{y})
  \]
  \begin{itemize}
    \item \(\mathbf{X}\boldsymbol{\theta} - \mathbf{y}\): residual vector.
    \item \(\mathbf{X}^T\): aggregates residuals by feature.
  \end{itemize}
\end{frame}
...
\begin{frame}{Convergence and Conditioning}
  \begin{itemize}
    ...
    \item Poorly conditioned \(\mathbf{X}^T \mathbf{X}\) slows convergence.
...
\begin{frame}{Gradient Descent Pseudocode}
  \begin{algorithm}[H]
  \SetAlgoLined
  \KwIn{Data matrix $\mathbf{X}$, targets $\mathbf{y}$, step size $\eta$, iterations $T$}
  \KwOut{Parameters $\boldsymbol{\theta}$}
  Initialize $\boldsymbol{\theta} \leftarrow 0$\;
  \For{$t=1$ \textbf{to} $T$}{
    $g \leftarrow \frac{2}{m} \mathbf{X}^T (\mathbf{X}\boldsymbol{\theta} - \mathbf{y})$\;
...
\begin{frame}{GD Variants: Gradient Estimates}
  ...
  \[
    g_t = \frac{2}{|\mathcal{B}_t|} \mathbf{X}_{\mathcal{B}_t}^T (\mathbf{X}_{\mathcal{B}_t}\boldsymbol{\theta} - \mathbf{y}_{\mathcal{B}_t})
  \]
  \begin{itemize}
...
\begin{frame}{Invertibility Conditions}
  \begin{itemize}
    \item \(\mathbf{X}^T \mathbf{X}\) invertible if columns of \(\mathbf{X}\) are linearly independent.
    \item If not full rank, use pseudo-inverse:
      \[
        \boldsymbol{\theta}^* = \mathbf{X}^{\dagger} \mathbf{y}
      \]
  \end{itemize}
\end{frame}
...
\begin{frame}{Polynomial Regression Example}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \node[anchor=west] at (axis cs:-1.8,1.6) {underfit};
      \node[anchor=west] at (axis cs:-0.2,2.0) {good fit};
      \node[anchor=west] at (axis cs:0.8,1.6) {overfit};
    \end{axis}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Training vs.\ Test Error Curves}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addlegendentry{Test}
      \addplot[dashed, color=black] coordinates {(0.5,0.2) (0.5,1.2)};
      \node[anchor=west] at (axis cs:0.08,1.05) {underfitting};
      \node[anchor=west] at (axis cs:0.72,1.05) {overfitting};
      \node[anchor=west] at (axis cs:0.52,0.35) {sweet spot};
    \end{axis}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{L2 Regularization (Ridge)}
  ...
  Closed form:
  \[
    \boldsymbol{\theta}^* = (\mathbf{X}^T \mathbf{X} + \lambda I)^{-1} \mathbf{X}^T \mathbf{y}
  \]
...
\begin{frame}{L1 Regularization (Lasso)}
  Objective:
  \[
    \min_{\boldsymbol{\theta}} \; \|\mathbf{X}\boldsymbol{\theta} - \mathbf{y}\|_2^2 + \lambda \|\boldsymbol{\theta}\|_1
  \]
  \begin{itemize}
...
\begin{frame}{L1 vs.\ L2: Geometry}
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    \fill[black] (1.1,1.0) circle (2pt);
    \node[blue] at (1.2,1.2) {$\ell_2$ ball};
    \node[red] at (-1.2,1.2) {$\ell_1$ ball};
    \node[black] at (1.9,0.2) {sparse corner};
    \node[black] at (1.6,1.0) {shrinkage};
  \end{tikzpicture}
\end{frame}
