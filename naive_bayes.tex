...
\begin{center}
\Large \textbf{Naive Bayes Classifier}
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \item For Spam ($y=1$):
    \begin{itemize}
        \item "buy": count=3, $\hat{\theta}_{1,\text{buy}} = (3+1)/(5+4) = 4/9 \approx 0.444$
        \item "cheap": count=2, $\hat{\theta}_{1,\text{cheap}} = (2+1)/(5+4) = 3/9 = 0.333$
        \item "meeting": count=0, $\hat{\theta}_{1,\text{meeting}} = (0+1)/(5+4) = 1/9 \approx 0.111$
        \item "tomorrow": count=0, $\hat{\theta}_{1,\text{tomorrow}} = (0+1)/(5+4) = 1/9 \approx 0.111$
    \end{itemize}
\end{itemize}
\end{frame}
...
\textbf{For Ham ($y=2$):}
\begin{itemize}
    \item "buy": count=0, $\hat{\theta}_{2,\text{buy}} = (0+1)/(5+4) = 1/9 \approx 0.111$
    \item "cheap": count=0, $\hat{\theta}_{2,\text{cheap}} = (0+1)/(5+4) = 1/9 \approx 0.111$
    \item "meeting": count=3, $\hat{\theta}_{2,\text{meeting}} = (3+1)/(5+4) = 4/9 \approx 0.444$
    \item "tomorrow": count=2, $\hat{\theta}_{2,\text{tomorrow}} = (2+1)/(5+4) = 3/9 = 0.333$
\end{itemize}
\pause
\textbf{Predict:} "buy cheap meeting"
\begin{align*}
\text{Score}_1 &= \log 0.5 + \log(4/9) + \log(3/9) + \log(1/9) \\
             &= -0.693 - 0.811 - 1.099 - 2.197 = -4.800 \\
\text{Score}_2 &= \log 0.5 + \log(1/9) + \log(1/9) + \log(4/9) \\
             &= -0.693 - 2.197 - 2.197 - 0.811 = -5.898
\end{align*}
\pause
$\hat{y} = 1$ (Spam) \checkmark
\end{frame}
...
% Class 1 mean and approximate region
\addplot[blue, fill=blue!20, fill opacity=0.3, only marks, mark=*, mark size=4pt] coordinates {
    (0.5,0.8) (1.2,1.1) (0.9,1.3) (1.1,0.7) (1.0,1.0)
};
\draw[blue, thick] (axis cs:1,1) circle [radius=1.5];
\node[blue] at (axis cs:1,1) {$\mu_1$};
% Class 2 mean and approximate region
\addplot[red, fill=red!20, fill opacity=0.3, only marks, mark=*, mark size=4pt] coordinates {
    (3.8,4.1) (4.2,3.9) (4.0,4.3) (3.9,4.0) (4.0,4.0)
};
\draw[red, thick] (axis cs:4,4) circle [radius=1.5];
\node[red] at (axis cs:4,4) {$\mu_2$};
% Decision boundary (linear in this case)
\addplot[black, dashed, very thick] coordinates {(2.5,-1) (2.5,6)};
\node[black] at (axis cs:2.5,5.5) {Decision boundary};
\legend{Class 1 samples, Class 2 samples}
\end{axis}
\end{tikzpicture}
\end{center}
\pause
\begin{itemize}
    ...
    \item Decision boundary at $x_1 = 2.5$ (midpoint between class means)
\end{itemize}
\end{frame}
