...
\section*{29. Next Token Prediction}
...
\section*{30. Clustering / PCA Short Questions}

\subsection*{30(a) K-Means Assignment}
...
\subsection*{30(b) K-Means Center Update}
...
\subsection*{30(c) Gaussian Mixture Responsibility}
Given two prior probabilities \texttt{pi1}, \texttt{pi2} and two likelihoods \texttt{p1}, \texttt{p2}, compute the responsibility of component 1:
\[
\gamma_1 = \frac{\pi_1 p_1}{\pi_1 p_1 + \pi_2 p_2}
\]

\begin{lstlisting}
def responsibility(pi1: float, p1: float, pi2: float, p2: float) -> float:
    pass
\end{lstlisting}
...
\begin{lstlisting}
def responsibility(pi1: float, p1: float, pi2: float, p2: float) -> float:
    return (pi1 * p1) / (pi1 * p1 + pi2 * p2)
\end{lstlisting}

\subsection*{30(d) Mean Centering for PCA}
...
\subsection*{30(e) PCA via Eigenvalue Decomposition in NumPy}
