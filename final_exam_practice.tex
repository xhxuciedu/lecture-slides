...
\section*{28. One Gradient Descent Step with Hinge Loss}
For one data point \texttt{(x, y)} with \texttt{y} in \(\{-1, +1\}\), perform one gradient descent update on \texttt{w} and \texttt{b} using the hinge loss \(\max(0, 1 - y \cdot (w^T x + b))\). The score is \(s = w^T x + b\). When the margin \(y s < 1\), the subgradient with respect to \texttt{w} is \(-y x\) and with respect to \texttt{b} is \(-y\); otherwise the subgradient is zero.

\begin{lstlisting}
def hinge_gradient_step(
    x: list[float],
    y: int,
    w: list[float],
    b: float,
...
def hinge_gradient_step(
    ...
    s = sum(wi * xi for wi, xi in zip(w, x)) + b
    if y * s >= 1:
        return w, b
    new_w = [wj + lr * y * xj for wj, xj in zip(w, x)]
    new_b = b + lr * y
    return new_w, new_b
\end{lstlisting}

\section*{29. Scaled Dot-Product Attention Score}
...
\section*{30. Scaled Dot-Product Attention Matrix}
...
\section*{31. Next Token Prediction}
...
\section*{32. Clustering / PCA Short Questions}

\subsection*{32(a) K-Means Assignment}
...
\subsection*{32(b) K-Means Center Update}
...
\subsection*{32(c) Gaussian Mixture Model: E-step and M-step}
...
\subsection*{32(d) Mean Centering for PCA}
...
\subsection*{32(e) PCA via Eigenvalue Decomposition in NumPy}
